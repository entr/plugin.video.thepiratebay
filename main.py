...
def search():
    ...
    option = settings.dialog.select(settings.string(32010), listTypes)
    url = "/search/%s/page/7/%s" % (query.replace(" ", "%20"), listUrl[option])
...
def nextPage(url="", page="0", showSeasons='True'):
    urlSearch = settings.value["urlAddress"] + url.replace('/page/', '/%s/' % int(page))
    ...
    titles = []
    urlSources = []
    links = soup.select("table#searchResult tr div.detName")
    for link in links:
        titles.append(link.a.text)

    links = soup.select("table#searchResult tr div + a")
    for link in links:
